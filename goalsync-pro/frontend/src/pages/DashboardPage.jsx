  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (user?.role === 'manager') promises.push(getTeamGoalsAPI({}));
        ...
        setData({
          ...
          teamGoals: user?.role === 'manager' ? extra || [] : [],
        });
      } catch (e) {
        console.error(e);
      } finally {
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.teamGoals.filter((g) => g.status === 'submitted').slice(0, 5).map((goal) => (
                    <tr key={goal.id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20 cursor-pointer" onClick={() => navigate(`/goals/${goal.id}`)}>
