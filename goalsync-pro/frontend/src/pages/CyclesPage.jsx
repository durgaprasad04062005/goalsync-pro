...
const CyclesPage = () => {
  ...
  const openEdit = (c) => { setForm({ ...c }); setEditCycle(c); setModal('form'); };

  const handleSave = async () => {
    if (!form.name || !form.startDate || !form.endDate) return toast.error('Name, start and end dates are required');
    setSaving(true);
    try {
      if (editCycle) {
        await updateGoalCycleAPI(editCycle.id, form);
  ...
  const handleActivate = async (cycle) => {
    setSaving(true);
    try {
      await updateGoalCycleAPI(cycle.id, { ...cycle, isActive: true });
      toast.success(`${cycle.name} is now the active cycle`);
      load();
    } catch (e) { toast.error('Failed to activate cycle'); }
    finally { setSaving(false); }
  };
    ...
    <Layout title="Goal Cycles" subtitle="Manage performance review cycles">
      <div className="space-y-5">
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cycles.map((cycle) => (
              <Card key={cycle.id} className={cycle.isActive ? 'ring-2 ring-primary-500' : ''}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{cycle.name}</h3>
                    {cycle.description && <p className="text-xs text-gray-400 mt-0.5">{cycle.description}</p>}
                  </div>
                  {cycle.isActive && (
                ...
                <div className="flex gap-2">
                  <Button size="xs" variant="outline" icon={PencilIcon} onClick={() => openEdit(cycle)}>Edit</Button>
                  {!cycle.isActive && (
      ...
      <Modal
        ...
        footer={<>
          ...
          <Button onClick={handleSave} loading={saving}>{editCycle ? 'Save Changes' : 'Create Cycle'}</Button>
        </>}
      >
        <div className="space-y-4">
          <Input label="Cycle Name" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. FY 2025-26" />
          <div className="grid grid-cols-2 gap-4">
            <Input label="Start Date" required type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} />
            <Input label="End Date" required type="date" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} />
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300">Set as active cycle (deactivates others)</span>
          </label>
        </div>
      </Modal>
    </Layout>
