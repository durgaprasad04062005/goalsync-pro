...
const AchievementsPage = () => {
  ...
  useEffect(() => {
    getGoalCyclesAPI().then(({ data }) => {
      setCycles(data.data);
      const active = data.data.find((c) => c.isActive);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const { data } = await getMyAchievementsAPI({ cycleId: selectedCycle || undefined });
            ...
            <div className="space-y-4">
              {quarterAchievements.map((ach) => (
                <motion.div
                  ...
                  onClick={() => navigate(`/goals/${ach.goalId}`)}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0 mr-3">
                      <p className="text-xs text-primary-600 dark:text-primary-400 font-medium mb-0.5">{ach.goal?.thrustArea}</p>
                      <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{truncateText(ach.goal?.title, 60)}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-3 text-xs">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 dark:text-gray-100">{ach.goal?.target}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900 dark:text-gray-100">{ach.goal?.weightage}%</p>
                    </div>
                  </div>
