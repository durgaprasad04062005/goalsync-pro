...
const CreateGoalPage = () => {
  ...
  const { myGoals } = useSelector((state) => state.goals);
  ...
  useEffect(() => {
    getGoalCyclesAPI().then(({ data }) => {
      const active = data.data.filter((c) => c.isActive);
      setCycles(active);
      if (active.length > 0) setForm((f) => ({ ...f, cycleId: active[0].id }));
    }).catch(() => {});
    dispatch(fetchMyGoals({}));
  }, [dispatch]);

  const currentCycleGoals = myGoals.filter((g) => g.cycleId === form.cycleId);
  const totalExistingWeightage = currentCycleGoals.reduce((s, g) => s + g.weightage, 0);
  ...
  const validate = () => {
    ...
    if (step === 2) {
      ...
      else if (w > remainingWeightage + 0.01) e.weightage = `Maximum available weightage is ${remainingWeightage.toFixed(1)}%`;
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center flex-1 last:flex-none">
              ...
              <span className={`ml-2 text-sm font-medium hidden sm:block ${i === step ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400'}`}>
                {s}
              </span>
              {i < STEPS.length - 1 && (
        ...
        <Card>
                ...
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      placeholder="Describe the goal in detail, success criteria, and approach..."
                      rows={4}
                  ...
                  {form.weightage && !errors.weightage && (
                    <div className="p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className={`font-semibold ${Math.abs(totalExistingWeightage + parseFloat(form.weightage || 0) - 100) < 0.01 ? 'text-green-600' : 'text-gray-900 dark:text-gray-100'}`}>
                      ...
                      <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full transition-all ${
                            totalExistingWeightage + parseFloat(form.weightage || 0) > 100 ? 'bg-red-500' : 'bg-primary-500'
                          }`}
              ...
              {step === 3 && (
                <>
                  ...
                  <div className="space-y-3">
                    {[
                      { label: 'Cycle', value: cycles.find((c) => c.id === form.cycleId)?.name },
                      ...
                      <div key={label} className="flex gap-4 py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
          ...
          <div className="flex justify-between mt-8 pt-5 border-t border-gray-100 dark:border-gray-700">
            <Button variant="secondary" onClick={step === 0 ? () => navigate('/goals') : back} icon={ChevronLeftIcon}>
              {step === 0 ? 'Cancel' : 'Back'}
            </Button>
            {step < STEPS.length - 1 ? (
              <Button onClick={next} iconRight={ChevronRightIcon}>Continue</Button>
            ) : (
              <Button onClick={() => handleSubmit(true)} loading={loading} icon={CheckIcon}>
                Save as Draft
              </Button>
            )}
          </div>
