...
import { UsersIcon, MagnifyingGlassIcon, CheckCircleIcon, ClockIcon } from '@heroicons/react/24/outline';
...
import { truncateText, getInitials, getId } from '../utils/helpers';
...
const TeamPage = () => {
  ...
  // Load cycles + team members first
  useEffect(() => {
    Promise.all([getGoalCyclesAPI(), api.get('/users/team')])
      .then(([cyclesRes, membersRes]) => {
        const list = cyclesRes.data.data || [];
        setCycles(list);
        setTeamMembers(membersRes.data.data || []);
        const active = list.find((c) => c.isActive);
        const cid = active ? (getId(active) || active._id) : (list[0] ? (getId(list[0]) || list[0]._id) : '');
  ...
  // Load team goals whenever cycle/filter changes — but only after initial load
  useEffect(() => {
    ...
    getTeamGoalsAPI(params)
      .then(({ data }) => setTeamGoals(data.data || []))
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [selectedCycle, statusFilter, selectedEmployee, initialLoaded]);

  const filtered = teamGoals.filter((g) => {
    if (!search) return true;
    const empName = `${g.employee?.firstName || ''} ${g.employee?.lastName || ''}`.toLowerCase();
    return g.title?.toLowerCase().includes(search.toLowerCase()) || empName.includes(search.toLowerCase());
  });
  ...
  const avgProgress      = teamGoals.length > 0
    ? (teamGoals.reduce((s, g) => {
        const last = g.achievements?.[g.achievements.length - 1];
        return s + (last?.progressPercentage || 0);
      }, 0) / teamGoals.length).toFixed(1)
    : 0;
      ...
      <div className="space-y-5">
        ...
        <Card>
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-48">
              ...
              <input
                type="text" placeholder="Search goals or employees..."
            ...
            <select value={selectedEmployee} onChange={(e) => setSelectedEmployee(e.target.value)} className="input-field text-sm w-44">
              ...
              {teamMembers.map((m) => (
                <option key={getId(m) || m._id} value={getId(m) || m._id}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {teamMembers.map((member) => {
            const memberId      = getId(member) || member._id;
            // goal.userId is a plain string (not populated in team goals)
            const memberGoals   = teamGoals.filter((g) => String(g.userId) === String(memberId));
            ...
            return (
              <motion.div
                key={memberId}
                whileHover={{ y: -2 }}
                onClick={() => setSelectedEmployee(isSelected ? '' : memberId)}
                className={`card p-4 cursor-pointer text-center transition-all ${isSelected ? 'ring-2 ring-primary-500' : ''}`}
              >
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((goal, i) => {
                    const gid     = getId(goal) || goal._id;
                    const lastAch = goal.achievements?.[goal.achievements.length - 1];
                    // employee is populated as goal.employee (set in controller) or goal.userId
                    const emp     = goal.employee || goal.userId || {};

                    return (
                      <tr key={gid || i} className="border-t border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20">
                        ...
                        <td className="py-3 px-4 w-32">
                          {lastAch
                            ? <ProgressBar value={lastAch.progressPercentage || 0} size="xs" showLabel={false} />
                            : <span className="text-xs text-gray-400">—</span>
                          }
                        </td>
                        <td className="py-3 px-4">
                          <Button size="xs" variant="ghost" onClick={() => navigate(`/goals/${gid}`)}>
                            View
                          </Button>
                        </td>
                      </tr>
